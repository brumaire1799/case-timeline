...
const C_TOP = '#CC0000', C_BOT = '#333333';
const pc = p => p === tp ? C_TOP : C_BOT;
const isTop = p => p === tp;
...
function slideHTML(evts, idx) {
  ...
  for (let i = 0; i < evts.length; i++) {
    const e = evts[i], c = pc(e.party), top = isTop(e.party), w = ws[i];
    cols += `
      <div class="col" style="flex:${w};--pc:${c};">
  ...
  return `<div class="slide${idx===0?' active':''}" data-p="${idx}">
    <div class="inner">
      <div class="hd"><h1>${esc(title)}</h1>
        <div class="lgd"><span style="--c:${C_TOP};">● ${esc(tp)}</span><span style="--c:${C_BOT};">● ${esc(bp)}</span></div>
...
.lgd{display:flex;gap:16px;flex-shrink:0}.lgd span{font-size:11px;font-weight:600;color:var(--c)}
...
.card{width:100%;min-height:2.5cm;border-radius:5px;padding:5px 7px;border-left:3px solid var(--pc);box-shadow:0 1px 3px rgba(0,0,0,.06)}
.ht .card{background:linear-gradient(135deg,#FFF5F5,#FFECEC)}
.hb .card{background:linear-gradient(135deg,#F8F8F8,#F0F0F0)}
...
.ndot.a{background:${C_TOP};transform:scale(1.4);box-shadow:0 0 6px rgba(204,0,0,.3)}
...
console.log(`✅ HTML: ${outputPath}  |  ${totalPages}页 ${events.length}事件  |  上:${tp}(红) 下:${bp}(黑)`);
